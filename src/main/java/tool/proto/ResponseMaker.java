...
public class ResponseMaker {
	...
	private void outputResponses(ProtoMessage message, String dir, String path, String protoPath) throws Exception {
		String javaName = JavaFilesMaker.firstUpper(message.getName().replace("SC_", "")) + "Response";
		...
		importBuilder.append(protoPath).append(".Response;");
		importBuilder.append("\r\n");
		importBuilder.append(protoPath).append(".Request;");
		importBuilder.append("\r\n");
		importBuilder.append(protoPath).append(".protocol.").append(message.getProtoName()).append("Protos.*;");
		importBuilder.append("\r\n");
		...
		for (ProtoMessageField field : message.getFields().values()) {
			...
			if (messages.containsKey(className)) {
				...
				if (!imports.containsKey(importFile)) {
					...
					importBuilder.append("\r\n");
					importBuilder.append(protoPath).append(".protocol.").append(importFile).append("Protos.*;");
				}
			}
		...
		builder.append(" */").append("\r\n");
		builder.append("public class ").append(javaName).append(" extends Response {");
		builder.append("\r\n");
